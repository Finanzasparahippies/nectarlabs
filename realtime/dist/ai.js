...
const buildNectarLabsServicesInfo = () => {
    return `
=======================================================
...
// ------------------------------------------------------------------------------
const buildNectarLabsSupportContext = async (client) => {
    try {
        ...
        if (tenantsRes.rows.length > 0) {
            ...
            for (const tenant of tenantsRes.rows) {
                ...
                context.push(`- Nombre de la Colmena: ${tenant.name}\n` +
        ...
        if (contractsRes.rows.length > 0) {
            ...
            for (const contract of contractsRes.rows) {
                ...
                const formattedNextPayment = contract.next_payment_date instanceof Date
                    ...
                    : 'No programado';
                context.push(`- Contrato #${contract.id} | Plan: ${planName}\n` +
...
const buildTenantSupportContext = async (chat, client, tenant) => {
    try {
        const ticketsRes = await pool.query('SELECT * FROM tickets_ticket WHERE client_id = $1 AND tenant_id = $2', [client.id, tenant.id]);
        const context = [];
        ...
        if (ticketsRes.rows.length > 0) {
            ...
            for (const ticket of ticketsRes.rows) {
                context.push(`- Ticket #${ticket.id} [${ticket.category}]: '${ticket.title}'\n` +
...
const buildSystemPrompt = async (chat, client, tenant) => {
    if (tenant) {
        ...
        return (`Eres el Asistente Virtual de Soporte Técnico de '${tenant.name}'.\n` +
        ...
        return (`Eres el Ingeniero de Soporte IA de Néctar Labs, un taller premium que diseña Software Artesanal: desde sistemas internos sencillos (CRM, administradores) hasta aplicaciones de altísima complejidad con toda la lógica de negocio, pasarelas de pago y automatizaciones.\n` +
...
// ------------------------------------------------------------------------------
export const generateAiReplyStream = async (chatId, newMsgText, onToken, onComplete, onError) => {
    ...
    try {
        ...
        const historyRes = await pool.query(`SELECT m.*, u.email as sender_email, u.role as sender_role 
       FROM tickets_supportchatmessage m 
       JOIN users_user u ON m.sender_id = u.id 
       WHERE m.chat_id = $1 
       ORDER BY m.created_at DESC LIMIT 15`, [chatId]);
