...
export default function FleetMap({ primaryColor, subdomain }: FleetMapProps) {
  ...
  return (
    <div className="bg-[#050a06]/40 border border-white/5 rounded-[2rem] p-6 shadow-lg relative overflow-hidden group text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-white/5 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-2xs uppercase tracking-widest font-black text-white/40">Portal de Rastreo y Logística en Tiempo Real</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1 bg-black/45 p-1 rounded-xl border border-white/5">
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-2xs font-black uppercase tracking-wider transition-all ${activePanelTab === 'tracking' ? 'bg-[#C68A1E] text-black' : 'text-white/60 hover:text-white'}`}
          >
            📍 Rastreo Map
          </button>
          <button 
            onClick={() => setActivePanelTab('fleet')} 
            className={`px-3 py-1.5 rounded-lg text-2xs font-black uppercase tracking-wider transition-all ${activePanelTab === 'fleet' ? 'bg-[#C68A1E] text-black' : 'text-white/60 hover:text-white'}`}
          >
            🚚 Flota
          </button>
          <button 
            onClick={() => setActivePanelTab('dispatch')} 
            className={`px-3 py-1.5 rounded-lg text-2xs font-black uppercase tracking-wider transition-all ${activePanelTab === 'dispatch' ? 'bg-[#C68A1E] text-black' : 'text-white/60 hover:text-white'}`}
          >
            📋 Despacho
          </button>
          <button 
            onClick={() => setActivePanelTab('telemetry')} 
            className={`px-3 py-1.5 rounded-lg text-2xs font-black uppercase tracking-wider transition-all ${activePanelTab === 'telemetry' ? 'bg-[#C68A1E] text-black' : 'text-white/60 hover:text-white'}`}
          >
            📡 Simulador GPS
          </button>
        </div>
      ...
      {message && (
        <div className={`mb-6 p-4 rounded-xl text-2xs font-bold uppercase tracking-wider text-center border ${message.type === 'success' ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
          {message.text}
        </div>
      )}
      ...
      {loading && (
        <div className="absolute top-4 right-4 flex items-center gap-1.5 bg-[#C68A1E]/10 border border-[#C68A1E]/30 px-3 py-1 rounded-full">
          ...
          <span className="text-2xs font-black uppercase tracking-widest text-[#C68A1E]">Sincronizando...</span>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="flex gap-2 items-center bg-black/30 border border-white/5 p-2 rounded-xl">
              <label className="text-2xs font-black uppercase tracking-widest text-white/40 pl-2">Seleccionar Vehículo:</label>
              <select 
                ...
                className="bg-black border border-white/10 rounded-lg text-2xs px-3 py-1 focus:outline-none focus:border-nectar-gold font-bold"
              >
            ...
            <div className="bg-[#020403] border border-white/5 rounded-2xl p-4 h-[300px] relative flex items-center justify-center overflow-hidden">
              ...
              {selectedStops.map((stop, idx) => {
                ...
                return (
                  ...
                  >
                    <span 
                      className={`w-3.5 h-3.5 rounded-full flex items-center justify-center text-2xs font-black border text-black shadow-md transition-all ${
                        isCompleted 
                    ...
                    <span className="absolute top-4 scale-0 group-hover/stop:scale-100 transition-all bg-black/90 border border-white/10 px-2 py-1 rounded text-2xs font-bold whitespace-nowrap text-white z-20">
              ...
              {selectedLocation ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-10 text-center p-6">
                  <p className="text-2xs font-black uppercase tracking-widest text-white/50">
              ...
              <div className="absolute bottom-3 left-3 right-3 flex justify-between bg-[#050a06]/85 backdrop-blur-md border border-white/5 px-3 py-1.5 rounded-lg text-2xs font-bold text-white/60">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-4.5 space-y-3 text-left">
              <div className="flex justify-between items-center">
                <span className="text-2xs font-black uppercase tracking-wider text-white/40">Estatus del Enlace</span>
                <span className="text-2xs font-black uppercase tracking-wider text-green-400 animate-pulse">Conectado en Vivo</span>
              </div>
              ...
              <div className="pt-2 border-t border-white/5 space-y-2">
                <div className="flex justify-between text-2xs">
                  <span className="text-white/40 uppercase font-black text-2xs">Conductor:</span>
                  <span className="text-white font-bold">{selectedVehicle?.driver_name || 'Sin asignar'}</span>
                </div>
                <div className="flex justify-between text-2xs">
                  <span className="text-white/40 uppercase font-black text-2xs">Placas:</span>
                  <span className="text-white font-mono font-bold">{selectedVehicle?.plate_number || 'N/A'}</span>
                </div>
                <div className="flex justify-between text-2xs">
                  <span className="text-white/40 uppercase font-black text-2xs">Estado del Vehículo:</span>
            ...
            <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-4.5 space-y-3 text-left">
              <h4 className="text-2xs font-black uppercase tracking-wider text-white border-b border-white/5 pb-2">Ruta de Entregas Programada</h4>
              {selectedStops.length === 0 ? (
                <p className="text-2xs text-white/30 italic">No hay paradas asignadas a este vehículo.</p>
              ) : (
                <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                  {selectedStops.map((stop) => (
                    <div key={stop.id} className="flex justify-between items-start gap-2 bg-white/[0.02] border border-white/5 p-2 rounded-lg text-2xs">
                      <div>
                        <p className="font-bold text-white">{stop.order}. {stop.name}</p>
                        <p className="text-2xs text-white/40 mt-0.5 truncate max-w-[130px]" title={stop.address}>{stop.address}</p>
                      </div>
            ...
            <button 
              onClick={loadData}
              className="w-full py-3 border border-white/10 hover:bg-white/5 rounded-xl text-2xs font-black uppercase tracking-widest text-white transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
            >
              🔄 Sincronizar Ahora
            </button>
          </div>
      ...
      {activePanelTab === 'fleet' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
          ...
          <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-6 space-y-4">
            <h4 className="text-2xs font-black uppercase tracking-wider text-white border-b border-white/5 pb-2">Registrar Nuevo Vehículo</h4>
            <form onSubmit={handleCreateVehicle} className="space-y-3">
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Nombre Identificador</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Placas del Vehículo</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Nombre del Chofer</label>
                <input 
                  type="text" 
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full py-3 bg-[#C68A1E] text-black text-2xs font-black uppercase tracking-widest rounded-xl hover:scale-[1.02] active:scale-95 transition-all cursor-pointer"
              >
          ...
          <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-6 space-y-4">
            <h4 className="text-2xs font-black uppercase tracking-wider text-white border-b border-white/5 pb-2">Vehículos Activos</h4>
            <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
              {vehicles.length === 0 ? (
                <p className="text-2xs text-white/30 italic">No hay vehículos registrados en este portal.</p>
              ) : (
                vehicles.map(v => (
                  <div key={v.id} className="flex justify-between items-center p-3 bg-white/[0.02] border border-white/5 rounded-xl text-2xs">
                    <div>
                    ...
                    <span className="px-2 py-0.5 bg-green-500/10 border border-green-500/20 text-green-400 rounded text-2xs font-black">
                      OPERATIVO
                    </span>
                  </div>
                ))
      ...
      {activePanelTab === 'dispatch' && (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 text-left">
          ...
          <div className="md:col-span-5 bg-white/[0.01] border border-white/5 rounded-2xl p-6 space-y-4">
            <h4 className="text-2xs font-black uppercase tracking-wider text-white border-b border-white/5 pb-2">Agregar Parada / Entrega</h4>
            <form onSubmit={handleCreateStop} className="space-y-3">
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Asignar al Vehículo</label>
                <select 
              ...
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Nombre del Destinatario / Parada</label>
                <input 
                  type="text" 
                  value={newStopName}
              ...
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Dirección Física</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-2xs font-black uppercase tracking-widest text-white/40">Latitud</label>
                  <input 
                    type="text" 
                    value={newStopLat}
                ...
                <div className="space-y-1">
                  <label className="text-2xs font-black uppercase tracking-widest text-white/40">Longitud</label>
                  <input 
                    type="text" 
                    value={newStopLon}
              ...
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Orden en la Ruta</label>
                <input 
                  type="number" 
                  value={newStopOrder}
          ...
          <div className="md:col-span-7 bg-white/[0.01] border border-white/5 rounded-2xl p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              <h4 className="text-2xs font-black uppercase tracking-wider text-white">Paradas Registradas</h4>
              
              {/* Selector in header to filter list */}
              <select
                value={selectedVehicleId || ''}
                onChange={(e) => setSelectedVehicleId(Number(e.target.value))}
                className="bg-black border border-white/10 rounded-lg text-2xs px-2 py-0.5 text-white"
              >
                {vehicles.map(v => (
                  <option key={v.id} value={v.id}>{v.name}</option>
                ))}
                ...
                selectedStops.map(st => (
                  <div key={st.id} className="p-3 bg-white/[0.02] border border-white/5 rounded-xl space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-2xs font-black text-white">{st.order}. {st.name}</p>
                        <p className="text-2xs text-white/40 mt-0.5 font-bold truncate max-w-[200px]" title={st.address}>{st.address}</p>
                    ...
                    <div className="flex justify-between items-center gap-2 pt-2 border-t border-white/5">
                      <div className="flex gap-1">
                        <button
                          ...
                          className={`px-2 py-1 rounded text-2xs font-black uppercase ${st.status === 'PENDING' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}
                        >
                          En Espera
                        </button>
                        <button
                          onClick={() => handleUpdateStopStatus(st.id, 'ARRIVED')}
                          className={`px-2 py-1 rounded text-2xs font-black uppercase ${st.status === 'ARRIVED' ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}
                        >
                          Llegó
                        </button>
                        <button
                          onClick={() => handleUpdateStopStatus(st.id, 'DEPARTED')}
                          className={`px-2 py-1 rounded text-2xs font-black uppercase ${st.status === 'DEPARTED' ? 'bg-white/20 text-white/40' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}
                        >
                          Entregó
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="px-2 py-1 bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500/20 rounded text-2xs font-black uppercase cursor-pointer"
                      >
                        Eliminar
                      </button>
                    </div>
      ...
      {activePanelTab === 'telemetry' && (
        <div className="max-w-xl mx-auto bg-white/[0.01] border border-white/5 rounded-2xl p-6 text-left space-y-4">
          <div>
            <h4 className="text-2xs font-black uppercase tracking-wider text-white">Transmisor GPS Satelital (Telemetría de Chofer)</h4>
            <p className="text-2xs text-white/40 uppercase tracking-widest mt-0.5">Actualización de coordenadas GPS en producción</p>
          </div>
          
          <form onSubmit={handleUpdateLocation} className="space-y-4 pt-2">
            <div className="space-y-1">
              <label className="text-2xs font-black uppercase tracking-widest text-white/40">Vehículo Transmisor</label>
              <select 
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Latitud Actual</label>
                <input 
                  type="text" 
                  value={simLat}
              ...
              <div className="space-y-1">
                <label className="text-2xs font-black uppercase tracking-widest text-white/40">Longitud Actual</label>
                <input 
                  type="text" 
                  value={simLon}
            ...
            <div className="bg-white/[0.02] border border-white/5 p-4 rounded-xl space-y-2">
              <h5 className="text-2xs font-black uppercase tracking-wider text-nectar-gold">💡 ¿Cómo funciona en producción?</h5>
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-4 bg-[#C68A1E] text-black text-2xs font-black uppercase tracking-widest rounded-xl hover:scale-[1.02] active:scale-95 transition-all cursor-pointer font-bold"
            >
