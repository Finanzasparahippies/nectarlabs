  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <div className="text-2xs font-black uppercase tracking-[0.4em] opacity-40">Procesando Métricas...</div>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="p-6 bg-card-bg border border-card-border rounded-[2rem] shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-2">
            <span className="text-2xs font-black uppercase tracking-widest opacity-40">Ganancias Totales</span>
            ...
            <p className="text-2xs text-foreground/50 mt-1 uppercase tracking-wider font-bold">Por entregas completadas</p>
          </div>
        </div>
        ...
        <div className="p-6 bg-card-bg border border-card-border rounded-[2rem] shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-2">
            <span className="text-2xs font-black uppercase tracking-widest opacity-40">Entregas Exitosas</span>
            <h3 className="text-3xl font-black text-green-400 font-mono">{completedOrders.length}</h3>
            <p className="text-2xs text-foreground/50 mt-1 uppercase tracking-wider font-bold">Paquetes entregados</p>
          </div>
        </div>
        ...
        <div className="p-6 bg-card-bg border border-card-border rounded-[2rem] shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-2">
            <span className="text-2xs font-black uppercase tracking-widest opacity-40">En Tránsito / Pendientes</span>
            <h3 className="text-3xl font-black text-yellow-500 font-mono">{transitOrders.length}</h3>
            <p className="text-2xs text-foreground/50 mt-1 uppercase tracking-wider font-bold">Activos en cola</p>
          </div>
        </div>
        ...
        <div className="p-6 bg-card-bg border border-card-border rounded-[2rem] shadow-xl relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-2">
            <span className="text-2xs font-black uppercase tracking-widest opacity-40">Entregas Fallidas</span>
            <h3 className="text-3xl font-black text-red-400 font-mono">{failedOrders.length}</h3>
            <p className="text-2xs text-foreground/50 mt-1 uppercase tracking-wider font-bold">No completadas</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 p-8 bg-card-bg border border-card-border rounded-[2.5rem] shadow-xl flex flex-col justify-between space-y-6">
          <div>
            <span className="text-2xs font-black uppercase tracking-widest text-nectar-gold">Desglose de Caja</span>
          ...
          <div className="p-4 rounded-2xl bg-yellow-500/10 border border-yellow-500/20 text-2xs leading-relaxed text-yellow-500/90 font-bold">
        ...
        <div className="lg:col-span-7 p-8 bg-card-bg border border-card-border rounded-[2.5rem] shadow-xl flex flex-col justify-between space-y-6">
          <div>
            <span className="text-2xs font-black uppercase tracking-widest text-nectar-gold">Rendimiento</span>
          ...
          <div className="h-44 flex items-end justify-between gap-2.5 pt-4">
            {weeklyData.map((d, idx) => (
              <div key={idx} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                {/* Tooltip on hover */}
                <div className="opacity-0 group-hover:opacity-100 bg-background border border-card-border px-2 py-1 rounded text-2xs font-mono text-nectar-gold transition-all duration-200 pointer-events-none transform -translate-y-1">
                ...
                <span className="text-2xs font-black uppercase tracking-widest text-white/40 group-hover:text-white transition-colors">
                  {d.day}
                </span>
              </div>
            ))}
      ...
      <div className="bg-card-bg border border-card-border rounded-[2.5rem] p-8 shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-card-border pb-6">
          <div>
            <span className="text-2xs font-black uppercase tracking-widest text-nectar-gold">Bitácora</span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-card-border/60 text-white/40 uppercase tracking-widest font-black text-2xs">
              ...
              {filteredOrdersList.map((o) => (
                <tr key={o.id} className="border-b border-card-border/40 hover:bg-white/[0.01] transition-colors">
                  ...
                  <td className="py-4 pr-4 text-right">
                    <span className={`inline-block text-2xs font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full ${
                      o.status === 'DELIVERED'
