  ...
  return (
    <section className="w-full py-32 px-6 max-w-7xl mx-auto" id="pricing">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        {plans.map((plan) => {
          const isRecommended = plan.name.toLowerCase().includes('mensual') || plan.name.toLowerCase().includes('premium') || plan.name.toLowerCase().includes('socio');
          
          return (
            <div
              key={plan.id}
              className={`p-12 rounded-[4rem] border-2 bg-card-bg flex flex-col justify-between hover:shadow-[var(--shadow-premium)] hover:-translate-y-4 transition-all duration-700 group relative overflow-hidden ${isRecommended ? 'border-nectar-gold shadow-2xl scale-105 z-10' : 'border-card-border shadow-lg'}`}
            >
              {isRecommended && (
              ...
              <Link href="/login" className="block w-full">
                <button className={`w-full py-8 rounded-[2rem] font-black uppercase tracking-widest transition-all text-sm shadow-xl ${isRecommended ? 'bg-nectar-gold text-nectar-cream hover:bg-nectar-forest hover:scale-[1.02]' : 'bg-nectar-forest text-nectar-cream hover:bg-nectar-gold'}`}>
                  Elegir Plan
                </button>
              </Link>
            </div>
